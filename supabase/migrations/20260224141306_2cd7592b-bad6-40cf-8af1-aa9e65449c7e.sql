...
BEGIN
  ...
  IF _prev_month_id IS NOT NULL THEN
    -- For each wallet that had a starting balance OR transactions in the previous month
    FOR _wallet IN
      SELECT DISTINCT w FROM (
        SELECT wallet AS w FROM public.wallet_starting_balances WHERE month_id = _prev_month_id
        UNION
        SELECT team AS w FROM public.transactions WHERE month_id = _prev_month_id AND team != 'CSB'
      ) sub
    LOOP
      -- Get previous starting amount (0 if no entry)
